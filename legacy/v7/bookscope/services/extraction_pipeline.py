"""Two-tier parallel extraction pipeline (v7).

Tier 1 (no LLM, <30s): emotion + style + arc analysis → immediate results
Tier 2 (LLM, ~3-5min): three-phase knowledge graph:
    Phase 1: full scan (ALL chunks, cheap model)
    Phase 2: smart selection (no LLM)
    Phase 3: deep analysis (selected chunks, strong model)
...
def run_extraction(
    ...
    while done_count < 2:
        try:
            event = q.get(timeout=600)  # 10 min timeout for v7 three-phase pipeline
        except queue.Empty:
