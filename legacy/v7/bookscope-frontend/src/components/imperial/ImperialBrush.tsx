      ...
      {characters.slice(0, 8).map((ch) => {
        ...
        return (
          <button
            ...
            className={clsx(
              "shrink-0 px-2.5 py-1 text-[11px] rounded border transition-colors cursor-pointer",
              "tracking-wide",
              active
                ? "border-[var(--vermillion)] text-[var(--vermillion)] bg-[var(--vermillion-light)]"
...
export default function ImperialBrush({
  ...
  // Listen for "ask about this section" events dispatched by section buttons.
  useEffect(() => {
    const handler = (e: Event) => {
      const detail = (e as CustomEvent<{ title?: string }>).detail;
      const title = detail?.title ?? "";
      const prefix = title ? `就【${title}】御问：` : "";
      setInput(prefix);
      setExpanded(false);
      // Focus and position cursor after prefix, then scroll input into view.
      setTimeout(() => {
    ...
    window.addEventListener("bookscope:ask-section", handler);
    return () => window.removeEventListener("bookscope:ask-section", handler);
  }, []);
