...
v3 = 输出字段不变(还是 v2 那套),但识别 prompt 改了:明确「政策黑话哪怕政界常见、对普通人也是
黑话」,把五年规划简称(十四五/十五五)、治理经济新概念(新质生产力/中国式现代化/高质量发展)、
数字缩略语(五位一体/四个全面)这类政界天天用但老百姓看不懂的词纳入该挑范围,同时仍守住别挑
通知/会议/单位这类大白话。识别层走 L2 缓存(key 含 system prompt),prompt 一改 key 自动变、
旧缓存自然 miss;这里同步把版本号 bump 到 v3,标明这层的产出口径变了。"""

DEFAULT_GLOSSARY_MAX_TOKENS = 8000
"""难词识别单段输出的 max_tokens。

**1800 是错的(live 抽查抓到)**:deepseek-v4-flash 把 reasoning_content 算进 max_tokens
(见 reference_reasoning_model_token_budget)。一段难词识别要先想一遍「哪些词外行看不懂、怎么用
人话讲」,reasoning 头很重;v2 加了语境含义 + 政策意图两层后输出更长,1800 被 reasoning 吃光、
content 返空 + finish_reason=length,整段抽不出一个词(公报实测 1800→0 词、4000→4 词截断、
8000→12 词 finish_reason=stop 抽全)。难词识别是单段、输入不大(一段几百到上千字),给够 token
不浪费——提到 8000(同条款维 ``DEFAULT_DOC_SPINE_MAX_TOKENS``),留足 reasoning 头还能把这段的
词都抽出来。"""
...
_INSTR_GLOSSARY = (
    ...
    "**重点:政策黑话哪怕在政界、新闻里天天出现,对普通老百姓也是黑话,该挑出来解释。**"
    "别因为「这词新闻里常见」就跳过,判断标准是**普通人懂不懂**,不是**政界用得多不多**。"
    "这几类尤其要挑:\n"
    "   - **五年规划等的简称 / 代称**:如「十四五」「十五五」(指第十四 / 十五个五年规划),"
    "普通人未必知道这是啥的简写。\n"
    "   - **治理 / 经济新概念、新提法**:如「新质生产力」「中国式现代化」「高质量发展」"
    "「新发展格局」「双循环」「碳达峰碳中和」「共同富裕」,这些是有特定含义的政策概念,"
    "不是字面意思,普通人说不清到底指什么。\n"
    "   - **数字缩略语**:如「五位一体」「四个全面」「两个确立」「三去一降一补」,"
    "光看数字加几个字根本不知道具体指哪几条。\n"
    ...
    "**平衡:多挑真黑话,但别挑大白话。**谁都懂的常见词(「通知」「会议」「单位」「召开」"
    "「研究」「同志」「工作」)、字面就能看懂的普通词,别挑,挑这些是噪声。只挑外行真要愣一下、"
    "不解释就不知道确切所指的词。一个真黑话都没有就返空数组,绝不为了凑数把大白话也塞进来。\n"
