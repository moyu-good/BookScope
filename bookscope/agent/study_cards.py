"""知识点卡片：据书出知识点卡片，每张含知识点 + 苏格拉底自测题 + 原文依据。

学习者发明区——拿书学东西，要可自测的卡片。长上下文整本进 context、列出书的知识点，
每张配一道苏格拉底式自测题（"先想后翻"）+ 一句原文依据。

scope：v1 是"卡片 + 自测题"（自测，不是 AI 实时多轮追问）；完整互动苏格拉底对话留后续
...
_SYSTEM_INSTRUCTION = (
    ...
    "卡片，每张给：知识点名、解释、一道苏格拉底式自测题（启发读者自己想，不是直接给答案）、"
