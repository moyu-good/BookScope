...
# 【exp-018 收紧,2026-07-02·两轮】弦外之音准确率 probe 在真语料测出原表假阳性 63%(#41 把命中
# meaning 提为逐条精读正文主体,一个误报=一句错白话)。**依托 empirical 人工核判、不靠估算**,撤 5 个:
#   · 撤「研究」单字(76%):多为"研究报告/课题/研究制定"名词或实义;留「研究研究」叠词、搁置义最强
#   · 撤「依法依规」「稳步」(各 100%):真语境全是中性"照章办事"程序词 / 默认修饰词
#   · 撤「原则上」(100%,第二轮):软化 meaning 没救——真公文"原则上X、特殊情况Y"把例外明写在后,
#     再标"有隐藏口子"是上下文取消式误读。它是最经典的弦外词,但在正式公文里恰恰不成信号,撤。
#   · 撤「一般」(57%,第二轮):信用一般/一般性等中性形态太多。
# 两轮后 empirical 假阳性 63%→38.8%→**目标 <20%**(留 8 个干净 marker:适时/原则同意/视情/相关部门确定
# 全 0,逐步 8%,结合实际 30% 但自由裁量真信号、聚合内可接受)。宁可漏不可错报。详见 experiments/018。
...
# 「研究」单字、「依法依规」「稳步」已按 exp-018 撤(见上;高假阳、真语境多为中性/实义)。
