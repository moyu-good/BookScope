...
_SYSTEM_INSTRUCTION = (
    ...
    "- 会议：会议记录 / 会议纪要 / 逐字稿。认它的标志——有参会人员（与会人 / 参会 / 出席）、"
    "主持人、记录人、会议时间、议程 / 议题，正文是一轮轮发言（带说话人）或按议题归并的纪要，"
    "末尾常有行动项 / 决议 / 待办。\n"
    "  **会议 vs 公文要分清**：会议是一场会的记录（谁说了什么、定了什么、谁去办），"
    "公文是机关发出的正式文件（有发文字号、发文机关、成文日期、印章这套红头要素，正文是"
    "条款式的通知 / 决定 / 批复）。看到「参会 / 主持人 / 记录人 / 发言 / 纪要」往会议归，"
    "看到「发文字号 / 红头 / 印章 / 第X条」往公文归。\n"
...
def is_narrative_genre(genre: str | None) -> bool:
    ...
    公文 / 会议 / 工具书 / 诗歌）返 ``False`` → 端点优雅退场，不在理论书上硬抽"人物关系"。
...
def genre_to_argument_axis(genre: str | None) -> str | None:
    ...
    - 其余（小说 / 历史 / 公文 / 会议 / 诗歌 / 工具书 / 其他）→ ``"fiction"``（论点结构优雅退场）
    """
    if genre is None:
        return None
