...
class TestChunkResult:
    ...
    def test_chapter_defaults_to_none(self):
        """老调用路径 / 老 AnalysisResult 里没有 chapter 字段时默认 None。"""
        cr = ChunkResult(index=0, text="text")
        assert cr.chapter is None

    def test_chapter_accepts_zero_for_prologue(self):
        """章节号 0 代表 detect_chapters 标识的"序"章节，合法值。"""
        cr = ChunkResult(index=0, text="prologue", chapter=0)
        assert cr.chapter == 0

    def test_chapter_accepts_positive_int(self):
        cr = ChunkResult(index=0, text="t", chapter=5)
        assert cr.chapter == 5

    def test_chapter_rejects_negative(self):
        import pydantic
        with pytest.raises(pydantic.ValidationError):
            ChunkResult(index=0, text="t", chapter=-1)

    def test_chapter_roundtrips_through_json(self):
        """老 JSON 没有 chapter 键 → None；新 JSON 有 chapter → 保留。"""
        new_json = ChunkResult(index=0, text="t", chapter=3).model_dump_json()
        restored = ChunkResult.model_validate_json(new_json)
        assert restored.chapter == 3

        legacy_payload = '{"index": 0, "text": "t", "word_count": 1}'
        restored_legacy = ChunkResult.model_validate_json(legacy_payload)
        assert restored_legacy.chapter is None
