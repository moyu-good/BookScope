...
def test_epub_metadata_title_wins_over_passed(tmp_path):
    # 2026-06-16 作者反馈"书名该从内容提取不是文件名"：epub 内容元数据 DC:title
    # 优先于传入 title（上传时传入的多半是文件名，不如出版物真名）。
    epub = _make_epub(tmp_path, "<p>Some text</p>", dc_title="Metadata Title")
    bt = load_text(epub, title="Filename Title")
    assert bt.title == "Metadata Title"


def test_epub_passed_title_is_fallback_when_no_metadata(tmp_path):
    # 无 DC:title 时退回传入 title（仍优先于文件名 stem）
    epub = _make_epub(tmp_path, "<p>Some text</p>", dc_title=None)
    bt = load_text(epub, title="Fallback Title")
    assert bt.title == "Fallback Title"
