...
读案例研究正文时，章节里频繁出现"ADR-007 决定切到 OpenAI function calling"、"按 ADR-006 删掉本地 reranker"这类引用。这些 ADR 自己就是 BookScope 工程路线的关节点，原文都躺在 `docs/architecture-decisions/`。附录 A 把 11 份 ADR 串成一份带摘要的索引，读者从章节回查时不用一份份点开看头。
