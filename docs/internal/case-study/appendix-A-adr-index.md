...
读案例研究正文时，章节里频繁出现"ADR-007 决定切到 OpenAI function calling"、"按 ADR-006 删掉本地 reranker"这类引用。这些 ADR 自己就是 BookScope 工程路线的关节点，原文都躺在 `docs/architecture-decisions/`。附录 A 把 12 份 ADR 串成一份带摘要的索引，读者从章节回查时不用一份份点开看头。
...
- **从"读书工具"到"读长文档引擎 + 可托管产品"**（后期两份大件）：ADR-010 把"整本读一次出带证据的脊、下游派生"定成架构，是后来公文 / 会议三根脊共用一台引擎的地基；ADR-011 把产品从"纯本地 BYOK、碰不到你任何东西"推成"可托管多用户、存你的文档但永不存 key"，是动了立身底线的代际级转向
...
## ADR-010 · 章脉共享索引（整本读一次，下游派生）

- **状态**：已批准 · 2026-06-23 作者签字
- **代际**：r2-agent-protocol
- **关联 commit**：`6dac162`（草案）/ `ed6413e`（作者签字 · 实现启动）/ `8c72931`（章脉单一事实源落地 · 整本一次精读出逐章带证据结构）

几百万字的书，十几个全书功能各自重读全文，等于把同一本书翻十几遍——又慢又贵。ADR-010 把这件事翻过来：整本精读**一次**，逐章产出带原文证据的结构化记录（叫"章脉"），下游叙事曲线 / 节奏 / 关系图 / 人物弧线全从这一份记录派生，不再各自重读。一次读、多处用，调用从 750 次量级压到 225 次，输入也不用发十遍。

这份 ADR 的分量不止在省钱。它定下的"脊"这个抽象——一份长文档读一次出带证据的结构、下游派生视图——后来被证明根本不是"读书"专用的。公文的"文脉"、会议的"会脉"全是同一个形状，换个分析单元（章 → 条款 → 议程项）照样跑。所以 ADR-010 名义上是个性能优化，实际是后面"一台引擎三根脊"整个架构的地基。

附录正文里 chapter-13（一台引擎，三根脊）从章脉讲到三根脊怎么共用一台引擎，章脉就是那台引擎的单一事实源。

## ADR-011 · 托管版账号后台 + 用户数据存储

- **状态**：已批准 · 2026-06-29 作者签字（moyu-good）
- **代际**：r2-agent-protocol
- **关联 commit**：`d5adcfd`（ADR-011 + NORTH_STAR 两形态数据底线）

BookScope 一直把"没有服务器、碰不到你的书、也碰不到你的 key"当立身招牌。但公网托管版有个硬伤：没账号，换设备或过段时间回来就得重传整本书、丢掉历史。要解决就只能在后端留住用户的文档——这跟"碰不到你的书"直接冲突。ADR-011 就是把这个底线让步摆上台面、由作者签字定下来的那份决定。

让步划得很死：**只让"碰不到你的书"半条，且只在用户主动选托管版时**——托管版存你的文档（加密 + 账号隔离 + 可彻底删除）。其它全守：碰不到你的 key（key 焊死客户端、按请求传、用完即弃，任何表都没有 key 字段）、evidence-first、BYOK、CPU-only、本地克隆版仍守全部底线无服务器。架构上靠一个 `BOOKSCOPE_DEPLOYMENT_MODE = local | hosted` 开关把两形态焊在一套代码里，本地版账号层全程旁路、逐字节零回归。

这份 ADR 也老实记了让步的代价：存用户文档 + 邮箱（PII）就成了《个保法》的个人信息处理者，要管加密存储、访问控制、留存 / 删除权、隐私政策——这是本决定最大的成本，不是服务器配置。

附录正文里 chapter-13 有一节专讲这次转向——"碰不到你任何东西"的产品怎么走到"可托管存你的文档"，一个开关怎么把两形态焊住而不分叉。

## ADR-012 · 通用深度文档分析引擎扩张（从书籍到任意长文档）

- **状态**：待作者签字（草稿 · 2026-06-29）
- **代际**：r2-agent-protocol（定位 / 场景代际，非架构代际）
- **关联**：追认 2026-06-24 NORTH_STAR 广义化 + 1.6 公文 / 1.7 会议已建；本 ADR 待签

追认性 ADR——把"BookScope 从书籍深读扩成通用长文档引擎"这个 2026-06-24 已拍板、NORTH_STAR 已改、公文（1.6）+ 会议（1.7）已建的代际级方向，补一份正式决策记录（2026-06-29 文档审计发现缺这份）。

它定的是**定位**:书籍是第一类、不是唯一,一台引擎（章脉 → 文脉 → 会脉）接多垂直、分析单元自适应（章 → 条款 → 议程项）,杀手价值在跨文件 / 跨单元;evidence-first / BYOK / CPU-only / 无真名等底线跨垂直全守。注意是**定位 / 场景代际,不是架构代际**——runtime 仍 r2、引擎仍章脉,没换地基。跟 ADR-010 是一对:010 给了"读一次出脊、下游派生"的引擎,012 给了"这引擎不只读书"的方向授权。

附录正文 chapter-13(一台引擎,三根脊)整章就是这个扩张的实证。
