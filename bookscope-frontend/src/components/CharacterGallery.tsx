      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
        {characters.map((ch) => (
          <button
            ...
            onClick={() =>
              navigate(
                `/book/${sessionId}/character/${encodeURIComponent(ch.name)}`,
              )
            }
            className="text-left bg-[var(--bg)] border border-[var(--border)] rounded-lg p-4 hover:border-[var(--accent)]/40 hover:bg-[var(--surface-hover)] transition-all duration-200 cursor-pointer group"
          >
